...
class BulkExtractTransitionSvc:
    """
    Demonstrates triggering the "extract" and "transition" workflow for a group of clusters or standalone hosts
    ...
    def run(self):
        ...
        extract_installed_image_spec = Inventory.ExtractInstalledImageSpec(entity_spec)
        apiClient = Inventory(self.config)

        # Call the extract task
        task = apiClient.extract_installed_image_task(extract_installed_image_spec)
        ...
        if (self.entityType.casefold() == "cluster".casefold()):
            ...
            clusters_to_be_transitioned = set()

            # Call the cluster summary API to check the eligible clusters
            ...
            print("----------Clusters that are eligible for transition----------")
            for summary in cluster_summaries:
                if summary.status == Clusters.TransitionStatus.ELIGIBLE:
                    print("Transition Eligible Cluster Details ---------- {}", summary)
                    # Store the clusters that can be transitioned.
                    clusters_to_be_transitioned.add(summary.cluster)
            print("----------Clusters that are not eligible for transition----------")
            for summary in cluster_summaries:
                if summary.status is not Clusters.TransitionStatus.ELIGIBLE:
                    print("Transition Non-eligible Cluster Details ---------- {}", summary)
            if clusters_to_be_transitioned:
                entity_spec = Inventory.EntitySpec(type=Inventory.EntitySpec.InventoryType.CLUSTER,
                                                   clusters=clusters_to_be_transitioned)
                # Create a spec for transition task
                ...
                if self.waitForTask(task):
                    clusterClient = Clusters(self.config)
                    params = Clusters.GetParams(type=Clusters.GetParams.InventoryType.CLUSTER, clusters=clusters_to_be_transitioned)
            ...
            hosts_to_be_transitioned = set()

            # Call the host summary API to check the eligible hosts
            host_summaries = hostClient.get(params).host_summaries

            # Hosts with status ELIGIBLE are eligible for transition, others are not
            print("----------Hosts that are eligible for transition----------")
            for summary in host_summaries:
                if summary.status == Hosts.TransitionStatus.ELIGIBLE:
                    print("Transition Eligible Host Details ---------- {}", summary)
                    # Store the hosts that can be transitioned.
                    hosts_to_be_transitioned.add(summary.host)
            print("----------Hosts that are not eligible for transition----------")
            for summary in host_summaries:
                if summary.status is not Hosts.TransitionStatus.ELIGIBLE:
                    print("Transition Non-eligible Host Details ---------- {}", summary)
            if hosts_to_be_transitioned:
                entity_spec = Inventory.EntitySpec(type=Inventory.EntitySpec.InventoryType.HOST,
                                                   hosts=hosts_to_be_transitioned)
                ...
                if self.waitForTask(task):
                    hostClient = Hosts(self.config)
                    params = Hosts.GetParams(type=Hosts.GetParams.InventoryType.HOST, hosts=hosts_to_be_transitioned)
...
def main():
    bulkExtractTransitionSvc = BulkExtractTransitionSvc()
    bulkExtractTransitionSvc.run()
